...
- 多重度（quantity）により、関数の引数が何回使用されるかを追跡できます。多重度 0 は、その引数が実行時に消去されることを意味します。
- 後から実装する予定のコードのプレースホルダーとして **ホール** を使用してください。REPL（またはエディタ）を使用して、ホールの型とそのコンテキストにあるすべての変数の名前・型・多重度を確認できます。
